...
围栏代码采用 Q 原型 B「工作台条带」：阅读视图有青色顶部条带、墨线、浅青纸面、无数字检查栏和短硬阴影，复制仍使用 Obsidian 原生按钮。长行在代码块内横向滚动；实时预览沿用 Obsidian 的折行和围栏编辑行为，并在原生语言或复制标记旁呈现相同配色。行内代码只使用紧凑的浅黄纸签；源码模式保留原始 Markdown 围栏。
