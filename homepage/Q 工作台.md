...
```dataviewjs
// Q 工作台只读 Vault 元数据。按钮仅执行本地 Obsidian 命令或打开笔记。
const host = dv.container;
host.classList.add("q-home");
const now = new Date();
...
} catch (error) { console.warn("Q 工作台：项目数据暂不可用", error); }

host.innerHTML = `
  <section class="q-home-hero" aria-labelledby="q-home-title">
    <div class="q-home-hero-copy"><p class="q-home-kicker">Q DESK / TODAY</p><h1 id="q-home-title">把今天放在<span>眼前</span>。</h1><p>任务、近期变化和常用入口，都在这一页。</p></div>
    <div class="q-home-hero-time" aria-label="打开工作台时的日期和时间"><strong>${timeLabel}</strong><span>${dateLabel}</span></div>
  </section>
  <div class="q-home-grid">
    <section class="q-home-card q-home-quick-card" aria-labelledby="q-home-quick-title"><header class="q-home-card-head"><div><h2 id="q-home-quick-title">快速入口</h2><small>常用插件</small></div></header><div class="q-home-actions"></div><p class="q-home-hint"></p></section>
    <section class="q-home-card q-home-tasks-card" aria-labelledby="q-home-tasks-title"><header class="q-home-card-head"><div><h2 id="q-home-tasks-title">待办与提醒</h2><small>Tasks / 当前 Vault</small></div><button type="button" class="q-home-link" data-command="obsidian-tasks-plugin:quick-search">打开 ↗</button></header><div class="q-home-task-filters" role="group" aria-label="任务范围"></div><div class="q-home-task-list"></div><button type="button" class="q-home-action q-home-task-more" data-command="obsidian-tasks-plugin:quick-search">打开完整任务列表 <span aria-hidden="true">↗</span></button></section>
    <section class="q-home-card q-home-projects-card" aria-labelledby="q-home-projects-title"><header class="q-home-card-head"><div><h2 id="q-home-projects-title">项目进度</h2><small>Project Manager</small></div><button type="button" class="q-home-link" data-command="project-manager:open-projects">打开 ↗</button></header><div class="q-home-project-list"></div></section>
    <section class="q-home-card q-home-vault-card" aria-labelledby="q-home-vault-title"><header class="q-home-card-head"><div><h2 id="q-home-vault-title">知识库概览</h2><small>Vault / 元数据</small></div></header><div class="q-home-metrics"></div></section>
    <section class="q-home-card q-home-recent-card" aria-labelledby="q-home-recent-title"><header class="q-home-card-head"><div><h2 id="q-home-recent-title">最近笔记</h2><small>按修改时间</small></div></header><div class="q-home-recent-list"></div></section>
    <section class="q-home-card q-home-week-card" aria-labelledby="q-home-week-title"><header class="q-home-card-head"><div><h2 id="q-home-week-title">本周日记</h2><small>日记 / Calendar</small></div><button type="button" class="q-home-link" data-command="calendar:show-calendar-view">打开日历 ↗</button></header><div class="q-home-week-days"></div><p class="q-home-week-state" role="status"></p></section>
  </div><p class="q-home-status" role="status" aria-live="polite"></p>`;
...
const addAction = (parent, label, command) => {
  const button = el("button", "q-home-action");
  ...
  button.append(el("span", "", label), el("span", "q-home-action-arrow", "↗")); parent.append(button);
};
const filters = host.querySelector(".q-home-task-filters");
...
function renderTasks() {
  ...
  const list = host.querySelector(".q-home-task-list"); list.replaceChildren();
  const rows = [...groups[selectedFilter]].sort((a, b) => (due(a) ?? scheduled(a) ?? "9999").localeCompare(due(b) ?? scheduled(b) ?? "9999"));
  host.querySelector(".q-home-grid").classList.toggle("has-sparse-tasks", rows.length < 3);
  host.querySelector(".q-home-tasks-card").classList.toggle("is-sparse", rows.length < 3);
  if (!rows.length) { list.append(el("p", "q-home-empty", selectedFilter === "today" ? "今天没有待处理任务。" : selectedFilter === "overdue" ? "没有逾期任务。" : "本周没有安排任务。")); return; }
  for (const task of rows.slice(0, 6)) {
    const row = el("button", "q-home-task-row"); row.type = "button"; row.dataset.path = task.path;
    row.append(el("span", "q-home-task-text", task.text.replace(/\s*[📅⏳🛫] ?\d{4}-\d{2}-\d{2}/g, "").trim()), el("small", "", taskMeta(task))); list.append(row);
  }
  if (rows.length > 6) list.append(el("p", "q-home-more-hint", `还有 ${rows.length - 6} 项，打开 Tasks 查看全部。`));
}
renderTasks();
const projectHost = host.querySelector(".q-home-project-list");
if (!pm) projectHost.append(el("p", "q-home-empty", "启用 Project Manager 后显示项目进度。"));
else if (activeProjects === null) projectHost.append(el("p", "q-home-empty", "项目状态暂不可用，可打开插件查看。"));
else if (!projects.length) projectHost.append(el("p", "q-home-empty", "当前没有进行中的项目任务。"));
else for (const project of projects) {
  const row = el("button", "q-home-project-row"); row.type = "button"; row.dataset.path = project.path;
  row.append(el("strong", "", project.title), el("small", "", `已完成 ${project.done}/${project.total} 项任务`)); projectHost.append(row);
}
const actions = host.querySelector(".q-home-actions");
...
host.querySelector(".q-home-hint").textContent = quickAddReady ? "" : "QuickAdd 尚无 Choice，先配置 Capture。";
const metrics = host.querySelector(".q-home-metrics");
for (const [value, label, tone] of [[allFiles.length, "笔记", "sun"], [tags, "标签", "cyan"], [changed, "近 7 日更新", ""], [activeProjects ?? "-", "进行中项目", ""]]) {
  const tile = el("div", `q-home-metric ${tone}`); tile.append(el("strong", "", value), el("span", "", label)); metrics.append(tile);
}
const recentHost = host.querySelector(".q-home-recent-list");
if (!recent.length) recentHost.append(el("p", "q-home-empty", "尚无笔记，创建后会在这里出现。"));
else for (const file of recent) {
  const row = el("button", "q-home-recent-row"); row.type = "button"; row.dataset.path = file.path;
...
const days = host.querySelector(".q-home-week-days"), weekState = host.querySelector(".q-home-week-state");
...
for (let index = 0; index < 7; index++) {
  ...
  const button = el("button", `q-home-day${date.toISODate() === todayKey ? " is-today" : ""}${file ? " has-note" : ""}`);
...
host.addEventListener("click", event => {
  ...
  const status = host.querySelector(".q-home-status");
